      ...
      <el-table-column
        ...
        label="名称">
              <template slot-scope="scope">
                <el-link  type="primary" :underline="false" @click="edit(scope.row.id)">{{scope.row.title}}</el-link>
              </template>
      </el-table-column>
      <el-table-column
        header-align="center"
        ...
        label="图标">
        <template slot-scope="scope">
          <i v-if="scope.row.icon" :class="scope.row.icon" aria-hidden="true"></i>
        </template>
      </el-table-column>
      <el-table-column
        prop="type"
...
export default {
  ...
  methods: {
    ...
    refreshList () {
      this.loading = true
      this.$http.get('/sys/menu/tree', { params: { topId: '1' } }).then(({ data }) => {
        this.loading = false
    ...
    // 修改
    edit (id) {
      this.$refs.menuForm.init('edit', { id: id, parentId: '' })
    },
    // 查看
    view (id) {
      this.$refs.menuForm.init('view', { id: id, parentId: '' })
    },
